...
class DLinkedL//our double limked list class
{
	...
	void DeleteAt(int pos)//delete At
	{
		...
		{
			Node *current,*temp=head; //new variable for traversing
...
int countItems()
	{
		...
		Node*current=head;// pointing current to head
		while(current!=NULL)
		{
			current=current->nxt;//jumping to next node
			i++;
		}
		return i;//returning the number of elements;
}
};
